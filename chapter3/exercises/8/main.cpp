#include <iostream> 
#include <vector>

/*
 *
 * EXERCISE 7 STUFF 
 *
 * */
constexpr int MIN_VALUE = 0;
constexpr int MAX_VALUE = 9;

std::vector<std::string>spelled_numbers {
  "zero",
  "one", 
  "two", 
  "three", 
  "four", 
  "five",
  "six", 
  "seven", 
  "eight", 
  "nine"
};

double char_to_double(const char c){
  return static_cast<int>(c) - '0';
}

double string_to_double(const std::string &s){
  for(int i = 0; i < spelled_numbers.size(); ++i){
    if (s == spelled_numbers[i]) return i;
  }

  std::cout << "Error: supplied invalid string to string_to_double()";
  exit(1);
}

bool validate_digit(const std::string &s){
  if(s.size() > 1) return false;

  int converted_char = char_to_double(s.at(0));

  return converted_char >= MIN_VALUE && converted_char <= MAX_VALUE;
}

bool validate_string(const std::string &input){
  for(std::string s : spelled_numbers){
    if (s == input) return true;
  }

  return false;
}

bool validate_input(const std::string &s){
  return validate_digit(s) || validate_string(s);
}

double get_numeric_value(const std::string &input){
  if(validate_digit(input)){
    return char_to_double(input.at(0));
  }

  if(validate_string(input)){
    return string_to_double(input) ;
  }
  return -1;
}
/*
 * CALCULATOR STUFF
 * */

constexpr char ADD_OPERATOR = '+';
constexpr char SUB_OPERATOR = '-';
constexpr char MUL_OPERATOR = '*';
constexpr char DIV_OPERATOR = '/';

bool validate_operator(const char &c){
  return (
    c == ADD_OPERATOR ||
    c == SUB_OPERATOR ||
    c == MUL_OPERATOR ||
    c == DIV_OPERATOR
  );
}

double calculate(const double &num1, const double &num2, const char &c){
  switch(c){
    case ADD_OPERATOR:
      return num1+num2; 
      break;
    case SUB_OPERATOR:
      return num1-num2;
      break;
    case MUL_OPERATOR:
      return num1*num2;
      break;
    case DIV_OPERATOR:
      return num1/num2;
  }

  return -1;
}

bool read_input(std::string &num1, std::string &num2, char &c){
  std::cout << "Please enter 2 numbers and an operator (+, -, *, /)" << std::endl;
  std::cin >> num1 >> num2 >> c;

  if (!validate_operator(c)) return false;
  if (!validate_input(num1) && !validate_input(num2)) return false;

  return true;
}

std::string get_operator_string(const char &c){
  switch(c){
    case ADD_OPERATOR:
      return "sum"; 
      break;
    case SUB_OPERATOR:
      return "subtraction";
      break;
    case MUL_OPERATOR:
      return "multiplication";
      break;
    case DIV_OPERATOR:
      return "division";
  }
  return "";
}

void show_result(const double &num1, const double &num2, const double &result, const char &op){
  std::string op_string = get_operator_string(op);
  std::cout << "\nThe " << op_string << " of " << num1 << " and " << num2 << " is " << result << "." << std::endl;
}

int main(){
  std::string s_num1, s_num2;

  double num1    {0}, 
         num2    {0},
         result  {0};
  char op {'0'};

  std::cout << "========== CALCULATOR ==========" << std::endl;

  if(!read_input(s_num1, s_num2, op)){
    std::cout << "\nError: Invalid Input" << std::endl;
    return -1;
  };

  num1 = get_numeric_value(s_num1);
  num2 = get_numeric_value(s_num2);
  
  //convert string to number here, then call calculate

  result = calculate(num1, num2, op);
  show_result(num1, num2, result, op);

  return 0;
}

// 1. Switch from double to string
// 2. Validate Strings as numbers
// 3. Convert Strings to numbers
// 4. Calculate
